...
class mod_sqli(Attack):
    ...
    def generate_payloads(self, html_code, payloads={}):
        ...
        for elem in e:
            ...
            if len(found_node) == 1:
                o[int(elem['lineno'])-1] = re.sub(r'(.*)<{0}>(.*)</{0}>(.*)'.format(elem['identifier']), lambda m: "{0}{1}{2}".format(m.group(1), self.payloads['payloads'][random.randint(0, 1)]['vector'].format(m.group(2)), m.group(3)), o[int(elem['lineno'])-1], flags=re.IGNORECASE)
